...
public class SuccinctUndirectedGraphSpeedTest {
	...
	public static void main(final String args[]) throws IOException, ClassNotFoundException {
        ...
        if (sparseFile.exists())
            sparse = (SparseIntUndirectedGraph) BinIO.loadObject(sparseFile);
		else {
        ...
        if (succinctFile.exists())
            ...
            succinct = new SuccinctUndirectedGraph(sparse);
        ...
        final GraphIterables<Integer, IntIntSortedPair> succinctIterables = succinct.iterables();
        ...
        for (final IntIntSortedPair e : succinctIterables.edges()) {
		...
		for (int k = 10; k-- != 0;) {
			...
			c = 0;
			for (int i = 1000000; i-- != 0;) {
                for (final IntIntSortedPair e : succinctIterables.outgoingEdgesOf(r.nextInt(n))) {
					u += succinct.getEdgeSource(e);
					u += succinct.getEdgeTarget(e);
					c++;
				}
